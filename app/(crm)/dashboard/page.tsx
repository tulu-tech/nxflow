...
import { Users, Target, Send, Zap, TrendingUp, Clock, Bell } from "lucide-react"
...
export default async function DashboardPage() {
  ...
  const [
    ...
    supabase.from("leadboard").select("*", { count: "exact", head: true }).eq("user_id", user.id),
    supabase.from("leadboard").select("*", { count: "exact", head: true }).eq("user_id", user.id).eq("status", "contacted"),
    supabase.from("leadboard").select("*", { count: "exact", head: true }).eq("user_id", user.id).eq("status", "converted"),
    supabase.from("leadboard").select("id, full_name, company, position, relevance_score, status, created_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(5),
    supabase.from("credit_usage").select("type, amount").eq("user_id", user.id).gte("created_at", monthStart).eq("type", "lusha_email"),
    supabase.from("email_campaigns").select("id, name, status, recipient_count, sent_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(3),
    supabase.from("follow_up_reminders").select("id, remind_at, note, lead_id, leadboard(full_name, company)").eq("user_id", user.id).eq("is_done", false).lte("remind_at", tomorrow).order("remind_at"),
  ])
  ...
  const remindersData = (dueReminders ?? []).map((r) => ({
    id: r.id,
    lead_id: r.lead_id,
    note: r.note,
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {/* Due Reminders Banner (client component for dismiss) */}
      {remindersData.length > 0 && (
        <DashboardReminders reminders={remindersData} />
      )}
          ...
          <CardContent className="pt-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-foreground mt-1">{totalLeads ?? 0}</p>
              </div>
          ...
          <CardContent className="pt-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-foreground mt-1">{contactedLeads ?? 0}</p>
              </div>
          ...
          <CardContent className="pt-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-foreground mt-1">{convertedLeads ?? 0}</p>
              </div>
          ...
          <CardContent className="pt-5">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-bold text-foreground mt-1">{lushaCreditsUsed}</p>
              </div>
